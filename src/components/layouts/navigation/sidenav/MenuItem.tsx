...
const MenuItem: React.FC<IMenuItem> = ({ isOpen, item }) => {
  const router = useRouter();
  const [openDropdown, setOpenDropdown] = useState(false);

  const normalLink =
    'group flex items-center justify-between px-6 py-3 text-md text-[#989EB3] hover:text-white hover:bg-[#181d2e] transition duration-0 ease-out hover:ease-in hover:duration-300';
  const activeLink =
